...
if ($login == 'admin') {
    ...
    $id_doctor = mysqli_fetch_all(mysqli_query($connect, "SELECT `id_doctor` FROM doctor WHERE `last_name` = '$last_name'
    AND `first_name` = '$first_name' AND `patronymic_name` = '$patronymic_name'"))[0][0];
    $id_user = mysqli_fetch_all(mysqli_query($connect, "SELECT `id_user` FROM doctor WHERE `id_doctor` = '$id_doctor'"))[0][0];
   



    $doctor_name = mysqli_fetch_all(mysqli_query($connect, "SELECT `first_name`, `last_name`, `patronymic_name` FROM doctor WHERE `id_user` = '$id_user' "));
    $doctor_first_name = $doctor_name[0][0];
    $doctor_last_name = $doctor_name[0][1];
    $doctor_patronymic_name = $doctor_name[0][2];

    $cabinet = mysqli_fetch_all(mysqli_query($connect, "SELECT `cabinet` FROM `doctor` WHERE `id_user` = '$id_user'"))[0][0];

    $cabinet = mysqli_fetch_all(mysqli_query($connect, "SELECT `name_cabinet` FROM `cabinet` WHERE `id_cabinet` = '$cabinet'"))[0][0];
...
<body>
    ...
    <div class="container">
        ...
        <nav class="navbar"> <!--Навигация-->
            <?php
    if ($specialization == 'Терапевт')
        echo '<form action = "patient_cout.php" method = "post">
            <input hidden name = "doctor_id" value = "' . $id_doctor . '">
            <input hidden name = "check_action" value = "output_assigned_patients">
            <button type = "submit"class="navbar_a">Прикрепленные пациенты</button>
            </form>';
            ?>
        ...
        <div class="info_doctor">
            ...
            <form class="profile_doctor" action="change_info_doctor.php" method="post">
                ...
                <div class="block_info">
                    ...
                    <input class="speciali_input" id="cabinet" type="text" name="cabinet" value="<?= $cabinet ?>"
                        readonly>
                    <div class="block_info">
                    ...
                    <div class="block_info">
                        ...
                        <input class="speciali_input" id="smena" type="text" name="smena" value="<?= $smena ?>"
                            readonly>
                    </div>
                    <div class="block_info">
...
$id_user = mysqli_fetch_all(mysqli_query($connect, "SELECT `id_user` FROM user WHERE `login` = '$login' AND `password` =
'$password'"))[0][0];
$id_doctor = mysqli_fetch_all(mysqli_query($connect, "SELECT `id_doctor` FROM doctor WHERE `id_user` =
'$id_user'"))[0][0];
...
$cabinet = mysqli_fetch_all(mysqli_query($connect, "SELECT `cabinet` FROM `doctor` WHERE `id_user` =
'$id_user'"))[0][0];

$cabinet = mysqli_fetch_all(mysqli_query($connect, "SELECT `name_cabinet` FROM `cabinet` WHERE `id_cabinet` =
'$cabinet'"))[0][0];
...
<body>
    ...
    if ($specialization == 'Терапевт')
        ...
        <div class="talone">
            <span class="info_title">Ваши приемы</span>
            <div class="talone_content">

                <form class="talone_block" action="talone_cout.php" method="post">
                    <div>
                        <span class="speciali_label_talone">Приемы на сегодня<span>
